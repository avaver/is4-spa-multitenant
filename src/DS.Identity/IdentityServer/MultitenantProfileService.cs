...
namespace DS.Identity.IdentityServer
{
    public class MultitenantProfileService : DefaultProfileService
    {
        ...
        public override async Task GetProfileDataAsync(ProfileDataRequestContext context)
        {
            ...
            {
                context.IssuedClaims.Add(new Claim(MultitenantClaimTypes.Tenant, user.TenantName));
                Logger.LogDebug("Added tenant claim {0} for user {1}", user.TenantName, user.UserName);
            }
        }
    }
}
